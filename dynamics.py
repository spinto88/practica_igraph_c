...
def dynamics(n = 100, f = 11, q = 20, virtual_links = 0, virtual_type = 0, phi = 0.00, zealots_fraction = 0.01, steps = 10000):

    libc = C.CDLL('src/libc.so')

    libc.graph_and_dynamics.argtypes = [C.POINTER(Axl_agent), \
                          C.c_int, C.c_int, C.c_int, C.c_double, C.c_int, C.c_int]
    ...
    libc.graph_and_dynamics(agents, dim, virtual_links, virtual_type, phi,\
